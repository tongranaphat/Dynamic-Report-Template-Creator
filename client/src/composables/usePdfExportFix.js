...
export function usePdfExportFix() {
  ...
  // Safe PDF generation with comprehensive error handling
  const generateHybridPdfSafe = async (canvasImages, projectData, variableMap = {}) => {
    ...
    try {
      ...
      // Process pages with error isolation
      ...
      for (let pi = 0; pi < numPages; pi++) {
        try {
          ...
          // Layer 1: Background image with CORS safety
          if (canvasImages[pi]) {
            try {
          ...
          // Layer 2: Vector text with font safety
  ...
  const addImageToPdfPage = async (pdfPage, obj) => {
    // Implementation for adding images to PDF
    // Similar error handling pattern as text
  };
